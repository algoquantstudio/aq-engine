...
By default it uses `BTCUSD` and validates account, ticker, and quote RPC calls without waiting for a completed bar.

```bash
cargo test --features runtime test_run_live_mt5_bridge_smoke -- --ignored --nocapture
```

To also enter the live loop and wait for subscribed bars:

```bash
export AQE_MT5_RUN_LIVE_LOOP=1
cargo test --features runtime test_run_live_mt5_bridge_smoke -- --ignored --nocapture
```
...
To include a tiny live order submission, enable the order test explicitly:

```bash
export AQE_MT5_ENABLE_ORDER_TEST=1
export AQE_MT5_TEST_QTY=0.01
cargo test --features runtime test_run_live_mt5_bridge_smoke -- --ignored --nocapture
```

To run the live MT5 broker/data-feed single-entry close test:
...
Only run the order path on an account and symbol where that volume is valid.
