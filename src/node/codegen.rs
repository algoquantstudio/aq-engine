...
fn node_input_string(node: &Node, name: &str, default: &str) -> String {
    node_input_value(node, name)
...
fn node_input_u8(node: &Node, name: &str, default: u8) -> u8 {
    node_input_value(node, name)
        .and_then(|value| value.as_i64())
        .and_then(|value| u8::try_from(value.max(1)).ok())
        .unwrap_or(default)
}

fn node_input_bool(node: &Node, name: &str, default: bool) -> bool {
    node_input_value(node, name)
        .and_then(|value| value.as_bool())
        .unwrap_or(default)
}

fn node_input_literal(input: &crate::node::NodeInput) -> String {
    match input.value.as_ref() {
        Some(serde_json::Value::String(value)) => format!(
            "\"{}\".to_string()",
            value.replace('\\', "\\\\").replace('"', "\\\"")
        ),
        Some(serde_json::Value::Number(value)) => format_number_literal(&input.input_type, value),
...
fn hyper_argument_source(input: &crate::node::NodeInput) -> String {
    let fallback = node_input_literal(input);
    match input.hyper_reference.as_deref() {
        Some(reference) => {
            let resolver = match input.input_type {
                crate::node::InputType::Int => "hyper_int",
                crate::node::InputType::Float => "hyper_float",
                crate::node::InputType::Bool => "hyper_bool",
                crate::node::InputType::Str => "hyper_string",
                _ => return fallback,
            };
            format!(
                "{}(ctx, \"{}\", {})",
...
fn timeframe_unit_variant(unit: &str) -> &'static str {
...
fn event_registration_from_node(node: &Node) -> EventRegistration {
    // `EventStreamType` currently supports Bar only. Keep the `event_type` input on
    // the node so the saved graph can evolve without changing the user-facing shape.
    let _event_type = node_input_string(node, "event_type", "Bar");
    EventRegistration {
        timeframe_amount: node_input_u8(node, "timeframe_amount", 15),
        timeframe_unit: timeframe_unit_variant(&node_input_string(
            node,
            "timeframe_unit",
            "Minute",
        ))
        .to_string(),
        allow_trading: node_input_bool(node, "allow_trading", false),
    }
}
...
pub fn generate_main_rs(meta: &StrategyMeta) -> Result<String, String> {
    meta.validate_hyper_parameters()?;
    ...
    for node_id in &sorted {
        ...
        for input in node.inputs.iter().filter(|i| {
            ...
            args.push(hyper_argument_source(input));
        }
        ...
        match node.node_type {
            ...
            NodeType::Trigger => {
                let is_on_start_event = reachable_connections
                    .iter()
                    .any(|conn| conn.to.node_id == node.id && conn.to.port == "on_start");
                if is_on_start_event {
                    event_registrations.push(event_registration_from_node(node));
                }
            }
            _ => {}
        }
    ...
    src.push_str("use aq_engine::prelude::*;\n");
    let mut hyper_helper_imports = std::collections::BTreeSet::new();
    for node in &meta.nodes {
        for input in &node.inputs {
            if input.hyper_reference.is_none() {
                continue;
            }
            let helper = match input.input_type {
                crate::node::InputType::Int => Some("hyper_int"),
                crate::node::InputType::Float => Some("hyper_float"),
                crate::node::InputType::Bool => Some("hyper_bool"),
                crate::node::InputType::Str => Some("hyper_string"),
                _ => None,
            };
            if let Some(helper) = helper {
                hyper_helper_imports.insert(helper);
            }
        }
    }
    if !hyper_helper_imports.is_empty() {
        src.push_str(&format!(
            "use aq_engine::core::strategy::hyperparameters::{{{}}};\n",
            hyper_helper_imports
                .into_iter()
                .collect::<Vec<_>>()
                .join(", ")
        ));
    ...
    // Register additional market-data event streams.
    for event in &event_registrations {
        src.push_str(&format!(
            "        ctx.add_events_with_options(EventStreamRequest::new(EventStreamType::Bar, Some(TimeFrame::new({}, TimeFrameUnit::{}))).allow_trading({}));\n",
            event.timeframe_amount,
            event.timeframe_unit,
            event.allow_trading
        ));
    }
    ...
    let has_hyperparameters = !meta.hyper_parameters.is_empty();
    let hyper_source_targets: Vec<serde_json::Value> = meta
        .nodes
        .iter()
        .flat_map(|node| {
            node.inputs.iter().filter_map(move |input| {
                input.hyper_reference.as_ref().map(|key| {
                    serde_json::json!({
                        "key": key,
                        "node_id": node.id,
    ...
    if has_hyperparameters {
        ...
        src.push_str(&format!(
            "    hyperparameters.set_sweep_id(\"{sweep_id}\").set_strategy_fingerprint(\"{sweep_id}\").set_source_targets(serde_json::from_str(r###\"{source_targets}\"###).expect(\"generated hyperparameter targets are valid\"));\n"
        ));
        for definition in &meta.hyper_parameters {
            let fallback = meta
                .nodes
                .iter()
                .flat_map(|node| node.inputs.iter())
                .find(|input| input.hyper_reference.as_deref() == Some(definition.name.as_str()))
                .and_then(|input| input.value.clone())
                .or_else(|| definition.values.first().cloned())
                .or_else(|| definition.min.map(serde_json::Value::from))
                .unwrap_or(serde_json::Value::Null);
            let parameter = serde_json::json!({
                "key": definition.name,
                "values": (!definition.values.is_empty()).then_some(&definition.values),
                "min": definition.min,
                "max": definition.max,
            ...
            let parameter_json =
                serde_json::to_string(&parameter).map_err(|error| error.to_string())?;
            src.push_str(&format!(
                "    hyperparameters.add_hyper_parameter(serde_json::from_str::<HyperParameter>(r###\"{parameter_json}\"###).expect(\"generated hyperparameter is valid\")).expect(\"generated hyperparameter registration is valid\");\n"
            ));
        }
        src.push_str("    let is_hyper_sweep = hyperparameters.is_sweep_requested(&args);\n\n");
    }
    src.push_str(&format!(
        "    let _ = set_logging_level(\"{}\");\n\n",
    ...
    if has_hyperparameters {
        src.push_str("        state.set_hyper_parameter_config(hyperparameters.clone());\n");
    }
    ...
    src.push_str("    } else {\n");
    if has_hyperparameters {
        src.push_str("        let hyper_selections_to_execute = hyperparameters.process_runs(&args).unwrap_or_else(|error| { eprintln!(\"{error}\"); std::process::exit(2); });\n        for selected_hyperparameters in hyper_selections_to_execute {\n");
    } else {
        src.push_str("        {\n");
    }
    src.push_str(&data_feed_init);
    ...
    if has_hyperparameters {
        src.push_str("        state.set_hyper_parameter_config(hyperparameters.clone());\n        if let Some(selection) = selected_hyperparameters.as_ref() { state.set_hyper_parameter_run(selection, is_hyper_sweep).unwrap_or_else(|error| { eprintln!(\"{error}\"); std::process::exit(2); }); }\n");
    }
    push_state_component_registrations(
        &mut src,
        &custom_lifecycle_logic,
    ...
    src.push_str("        if let Err(e) = state.run_backtest(start, end, timeframe).await {\n");
...
mod tests {
    ...
    #[test]
    fn generated_backtest_save_is_engine_managed() {
        ...
        assert!(src.contains("use aq_engine::prelude::*;"));
        assert!(!src.contains("use aq_engine::core::alpha::{"));
        assert!(!src.contains("use aq_engine::core::pipeline::{"));
        ...
        assert!(!src.contains("RESULTS_SAVED_TO"));
        assert!(!src.contains("aqmeta_copied"));
        assert!(!src.contains("strategy.aqmeta"));
        assert!(!src.contains("\"run_config\""));
        assert!(!src.contains("HyperParameterConfig"));
        assert!(!src.contains("--hyper-sweep"));
    }

    #[test]
    fn generated_hyperparameter_runtime_is_engine_managed() {
        let mut meta = StrategyMeta::new("Hyperparameter Codegen Test");
        meta.nodes
            .push(StrategyMeta::create_strategy_node(&meta.name));
        meta.hyper_parameters
            .push(crate::node::HyperParameterDefinition {
                name: "period".to_string(),
                value_type: crate::node::HyperParameterType::Int,
                values: vec![serde_json::json!(7), serde_json::json!(14)],
                min: None,
                max: None,
                increment: None,
            });
        ...
        assert!(src.contains("HyperParameterConfig"));
        assert!(src.contains("state.set_hyper_parameter_config(hyperparameters.clone())"));
        assert!(src.contains("state.set_hyper_parameter_run(selection, is_hyper_sweep)"));
        assert!(!src.contains("\"runs\":"));
        assert!(!src.contains("hyperparameters::{"));
        assert!(!src.contains("fn hyper_values("));
        assert!(!src.contains("fn hyper_int<"));
        assert!(!src.contains("set_backtest_result_dir("));
        assert!(!src.contains("metrics.json"));
        assert!(!src.contains("__HYPER__\").to_string(), values"));
    }
    ...
    #[test]
    fn generated_add_event_node_registers_feature_timeframe() {
        let mut meta = StrategyMeta::new("Add Event Test");
        meta.nodes
            .push(StrategyMeta::create_strategy_node(&meta.name));
        meta.nodes.push(custom_node(
            "add-event",
        ...
        assert!(src.contains("EventStreamRequest"));
        assert!(src.contains("EventStreamType::Bar"));
        assert!(src.contains(
            "ctx.add_events_with_options(EventStreamRequest::new(EventStreamType::Bar, Some(TimeFrame::new(15, TimeFrameUnit::Minute))).allow_trading(true));"
        ));
        assert!(
            src.find("ctx.add_events_with_options").unwrap()
                < src
                    .find("ctx.add_alpha")
                    .unwrap_or(src.find("fn init").unwrap())
        );
    }
